...
def query(query, index, chunks, top_k: int = 5):
    ...
    print("query embed shape o query.py la ", query)
    distances, indices = search_faiss(index, query_embedding, k=top_k)
    final_indices = [indices[0][i] for i in range(len(indices[0]))  if distances[0][i] >= 20] # Threshold = 20, chỉ có chunks nào có điểm L2 lớn hơn 15 được cho vào, tránh trường hợp sại lệch do thiếu data mà vẫn phải chọn top_k
